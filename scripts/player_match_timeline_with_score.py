...
duckdb.sql(f"""
                      with score_info as (
                       ...
                       --POSITION_SIDE_ADJ, 
                       POSITION_TYPE, POSITION_TYPE_ALT, POSITION_BEHAVIOR, --PLAYERS_SAME_COUNTRY, 
                      ...
                      -- WHEN IFNULL(LAG(POSITION_SIDE_ADJ,1) OVER (PARTITION BY track_times.match_id, track_times.period, player_id ORDER BY track_times.match_id, track_times.period, player_id, track_times.interval_start),'N/A') != POSITION_SIDE_ADJ THEN 1
                       ...
                       LEFT JOIN score_info
                        ...
                        FROM (
                        SELECT match_id, period, team_id, TEAM_GOALS, OPP_GOALS, player_id, --country_id, 
                        --POSITION_SIDE_ADJ, 
                        POSITION_TYPE, 
                        POSITION_TYPE_ALT, POSITION_BEHAVIOR, --PLAYERS_SAME_COUNTRY, 
                       --PLAYERS_DIFF_COUNTRY, POSITION_SAME_COUNTRY, POSITION_DIFF_COUNTRY, SIDE_SAME_COUNTRY, SIDE_DIFF_COUNTRY, 
                       interval_start
                       FROM id_changes
                       WHERE iso_row = 1
...
print('Player Match Timeline with Score Done.')