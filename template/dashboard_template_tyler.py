...
wc_2026_info = pl.read_csv(ADDITIONAL_DIR / "WC_Data/WC_2026.csv")
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                html.Div(
                                    [
                                        html.Label("Select Team One", style=LABEL_STYLE),
                                        dcc.Dropdown(
                                ...
                                html.Div(
                                    [
                                        html.Label("Team Two", style=LABEL_STYLE),
                                        dcc.Dropdown(
                                            id="season-filter",
                                            options=[
                                                {"label": "Select Team Two", "value": "all"}
                                            ]
                                            + [
                                                {"label": comp, "value": comp}
                                                for comp in sorted(
                                    ...
                                    children=[
                                        dcc.Graph(
                                            figure=dash_table.DataTable(
                                                id='table_one',
                                                columns=[{"name":i, "id":i} for i in wc_2026_info.columns],
                                                data = wc_2026_info,
                                                #x="shot_statsbomb_xg",
                                                #nbins=30,
                                                #labels={
...
def update_results_chart(competition, season):
    filtered_df = wc_2026_info

    if competition != "all":
        filtered_df = filtered_df.filter(pl.col("competition_name") == competition)
    if season != "all":
        filtered_df = filtered_df.filter(pl.col("season_name") == competition)
    # if team != "all":
    #     filtered_df = filtered_df.filter(
    #         (pl.col("home_team") == team) | (pl.col("away_team") == team)
    #     )

    result_counts = filtered_df.group_by("Country").agg(pl.len().alias("count"))

    fig = go.Figure(data=[go.Table(
        header = dict(values=(list(result_counts))),
        cells=dict(values=[result_counts.count])
        )]
