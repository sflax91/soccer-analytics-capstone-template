...
duckdb.sql(f"""
                      ...
                      strptime('2026-01-01' , '%Y-%m-%d') + TO_MINUTES(minute) + TO_SECONDS(second) event_date
                        ...
                        LEFT JOIN (                        
                          ...
                          LEFT JOIN (                        
                           ...
                           LEFT JOIN (
                            ...
                            AND carry_events.period = score_check1.start_period
                           ...
                           LEFT JOIN (
                            ...
                            ON carry_events.match_id = score_check2.match_id
                            AND (
                                  (carry_events.period = score_check2.start_period AND carry_events.event_date >= score_check2.SCORE_TL_START)
                                  OR 
                                  (carry_events.period = score_check2.end_period AND carry_events.event_date < IFNULL(score_check2.SCORE_TL_END, strptime('2027-01-01' , '%Y-%m-%d')))
                                  OR 
                                  (carry_events.period > score_check2.start_period AND carry_events.period < score_check2.end_period)
                                  )
                            ...
                            ON carry_events.match_id = score_check3.match_id
                            AND (
                                  (carry_events.period = score_check3.start_period AND carry_events.event_date >= score_check3.SCORE_TL_START)
                                  OR 
                                  (carry_events.period > score_check3.start_period)
                                  )
                        )
                        ...
                        FROM get_score
                    """).write_parquet('carry_level_stats.parquet')
